...
`catalog.sqlite` 还保存 REST 请求的无效标识排除记录，包括数据集、标识、endpoint、
错误码、错误信息、首次/最近失败时间、失败次数和后续跳过次数。排除记录绑定合约
元数据指纹；`uly`、上市时间、到期时间或关联的合约集合变化后自动失效并重新探测。
...
- `2020-01-02` 至 `2023-06-30` 使用
  `GET /api/v5/market/history-candles`，逐合约倒序分页；
- `2023-07-01` 起使用 OKX 历史数据模块 `2` 的批量文件；
- REST 与批量文件出现重叠主键时必须内容一致，最终按
  `venue, instrument_id, bar_type, timestamp` 去重；两个来源边界上的共同合约必须
  保持 1 分钟连续。
...
价格 K 线请求按合约 `listTime`/`expTime` 与目标 UTC 日的交集筛选。未上市合约只跳过
上市前的日期，不进入无效标识排除表；到达上市日后自动开始请求。指数 K 线的
...
| 普通 1m K 线 | REST 官方只承诺“recent years”；批量离线文件从 2023-07 起 | `2020-01-02` 至 `2023-06-30` 逐合约走 REST，`2023-07-01` 起走批量文件 |
...
- 普通 K 线 REST 同样没有对所有合约承诺统一的 2020 起点；程序按 catalog 中的
  上市/到期窗口请求，并以接口实际返回的首条记录为准。当前公共 instruments 接口
  不能完整枚举早已下架的交割合约，因此这类合约只有在 catalog 已通过历史快照或
  本地数据发现时才会参与 REST 回补。
...
1. 批次 A：普通 K 线，`2020-01-02` 至 `2023-06-30` 逐合约走 REST，
   `2023-07-01` 至 mature_end 走批量文件；
